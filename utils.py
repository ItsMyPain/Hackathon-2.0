...
def predict_yandex(instruction_text: str, request_text: str) -> str:
    ...
    body = {
        "model": "general",
        "instructionText": "Ты занимаешься подготовкой и переформулировкой запроса из финансовой области в более понятную для нейросетей запрос. Обработанный запрос должен содержать не более 10 предложений",
...
Исправленный запрос должен содержать роль респондента (например, как эксперт в какой-либо области, как аналитик, как программист и так далее).
Исправленный запрос должен содержать 5 дополнительных фраз - набор простых неповторяющихся вопросов по этой теме.
Кроме того, исправленный запрос должен передавать смысл исходного запроса: '{text}'"""
